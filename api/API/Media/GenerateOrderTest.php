...
function insertOrderData($conn, $orderParams)
{
    try {
        $stmtOrderTable = $conn->prepare(
            ...
            (`EntryDate`, `EntryUser`, `OrderNumber`, `RateWiseOrderNumber`, `RateID`, `OrderDate`, 
            `DateOfFirstRelease`, `DateOfLastRelease`, `ClientName`, `ClientContact`, `Source`, `Owner`, 
            `CSE`, `Receivable`, `Payable`, `CardRatePerUnit`, `Margin`, `VendorName`, `AdCategory`, `AdType`, 
            `AdWidth`, `AdHeight`, `ClientAuthorizedPerson`, `BookedStatus`, `Units`, `SearchTerm`) 
            VALUES 
            (CURRENT_DATE, :PDOEntryUser, :PDOOrderNumber, :PDORateWiseOrderNumber, :PDORateId, CURRENT_DATE, 
            DATE_ADD(CURRENT_DATE, INTERVAL 1 DAY), DATE_ADD(CURRENT_DATE, INTERVAL 1 DAY), :PDOClientName, 
            :PDOClientContact, :PDOSource, :PDOOwner, :PDOCSE, :PDOReceivable, :PDOPayable, 
            :PDOCardRatePerUnit, :PDOMargin, :PDOVendorName, :PDOAdCategory, :PDOAdType, 
            :PDOAdWidth, :PDOAdHeight, :PDOClientAuthorizedPerson, :PDOBookedStatus, :PDOUnits, :PDOSearchTerm)"
        );
...
try {
    ...
    if ($status === 'Won') {
        $orderParams = [
            ...
            'PDORateWiseOrderNumber' => $_GET['JsonRateWiseOrderNumber'] ?? null,
            ...
            'PDOAdHeight' => $_GET['JsonAdHeight'] ?? null,
        ...
        // Build SearchTerm
        $orderParams['PDOSearchTerm'] = trim($orderParams['PDOOrderNumber'])
            . ($orderParams['PDORateWiseOrderNumber'] !== null ? " - " . trim($orderParams['PDORateWiseOrderNumber']) : '')
            . ($orderParams['PDOOrderDate'] ?? '')
            . ($orderParams['PDOClientName'] !== null ? " - " . trim($orderParams['PDOClientName']) : '')
            . ($orderParams['PDOReceivable'] !== null ? " - ₹" . trim($orderParams['PDOReceivable']) : '');
